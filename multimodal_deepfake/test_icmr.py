...
import utils
from dataset import create_sampler, create_loader, create_test_dataset
...
def evaluation(args, model, data_loader, tokenizer, device, config):
    ...
    for i, (image, label, text, W, H) in enumerate(metric_logger.log_every(args, data_loader, print_freq, header)):
        fake_image_box = 0
        fake_word_pos = 0
        fake_token_pos = 0
        ...
        real_label_pos = np.where(np.array(label) == 0)[0].tolist()
    ...
    print(cls_acc_all, cls_nums_all)
    ...
    return AUC_cls, ACC_cls, EER_cls, F1_multicls
...
def main_worker(gpu, args, config):
    ...
    eval_type = os.path.basename(config['test_file'][0]).split('.')[0]
    if eval_type == 'test':
        eval_type = 'all'
    ...
    if os.path.exists(f'{args.output_dir}/{args.log_num}')==False:
        os.makedirs(f'{args.output_dir}/{args.log_num}', exist_ok=True)
    checkpoint_dir = f'{args.output_dir}/{args.log_num}/HAMMER_checkpoint_{args.test_epoch}.pth'
    ...
    val_dataset = create_test_dataset(config)
    ...
    AUC_cls, ACC_cls, EER_cls, F1_multicls = evaluation(args, model_without_ddp, val_loader, tokenizer, device, config)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='./configs/test_icmr.yaml')
